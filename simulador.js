    ...
    for (let j = 0; j < bodies.length; j++) {
      if (i !== j) {
        ...
        if (distance > MIN_DISTANCE) {
          let direction = new THREE.Vector3().subVectors(other.position, body.position).normalize();
          ...
          let repulsion = new THREE.Vector3().subVectors(body.position, other.position).normalize().multiplyScalar(1000);
          force.add(repulsion);
        }
      }
    }
...
// Creación de cuerpos
const bodies = [
  new Body(new THREE.Vector3(-5, 0, 0), new THREE.Vector3(0, 0, 0), 5, 0xff0000),
...
function addScrollListeners() {
  ...
  document.getElementById("scrollX1").addEventListener("input", () => {
    bodies[0].position.x = parseFloat(document.getElementById("scrollX1").value) || 0;
    bodies[0].updateMesh();
  });

  document.getElementById("scrollY1").addEventListener("input", () => {
    bodies[0].position.y = parseFloat(document.getElementById("scrollY1").value) || 0;
    bodies[0].updateMesh();
  });

  document.getElementById("scrollZ1").addEventListener("input", () => {
    bodies[0].position.z = parseFloat(document.getElementById("scrollZ1").value) || 0;
    bodies[0].updateMesh();
  });

  // Cuerpo 2
  document.getElementById("scrollX2").addEventListener("input", () => {
    bodies[1].position.x = parseFloat(document.getElementById("scrollX2").value) || 0;
    bodies[1].updateMesh();
  });

  document.getElementById("scrollY2").addEventListener("input", () => {
    bodies[1].position.y = parseFloat(document.getElementById("scrollY2").value) || 0;
    bodies[1].updateMesh();
  });

  document.getElementById("scrollZ2").addEventListener("input", () => {
    bodies[1].position.z = parseFloat(document.getElementById("scrollZ2").value) || 0;
    bodies[1].updateMesh();
  });

  // Cuerpo 3
  document.getElementById("scrollX3").addEventListener("input", () => {
    bodies[2].position.x = parseFloat(document.getElementById("scrollX3").value) || 0;
    bodies[2].updateMesh();
  });

  document.getElementById("scrollY3").addEventListener("input", () => {
    bodies[2].position.y = parseFloat(document.getElementById("scrollY3").value) || 0;
    bodies[2].updateMesh();});
  
  document.getElementById("scrollZ3").addEventListener("input", () => {
      bodies[2].position.z = parseFloat(document.getElementById("scrollZ3").value) || 0;
      bodies[2].updateMesh();
    });
  }
  ...
  document.getElementById("simulator-button").addEventListener("click", () => {
    ...
    bodies[0].velocity.set(parseFloat(document.getElementById("velocidad1").value) || 0, 0, 0);
    bodies[1].mass = parseFloat(document.getElementById("masa2").value) || 1;
    bodies[1].velocity.set(parseFloat(document.getElementById("velocidad2").value) || 0, 0, 0);
    bodies[2].mass = parseFloat(document.getElementById("masa3").value) || 1;
    bodies[2].velocity.set(parseFloat(document.getElementById("velocidad3").value) || 0, 0, 0);
    bodies.forEach((body) => body.updateMesh());
